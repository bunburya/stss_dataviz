...
The data published by ESMA includes certain details about the *originators* in respect of each STS securitisation, but it does not currently include details of the *issuers* (which may be based in different countries to the respective originators).  However, where debt securities are listed on a stock exchange in the European Economic Area (EEA), the exchange is required under EU law to submit details of those debt securities to ESMA.  ESMA publishes that data in its [Financial Instrument Reference Data System](https://registers.esma.europa.eu/publication/searchRegister?core=esma_registers_firds) (FIRDS).  By cross-referencing the International Securities Identification Numbers (ISINs) associated with public securitisations against FIRDS data, we are able to piece together some additional information about the relevant securities and the entities that issue them.
...
Finally, we break down the public STS securitisations by the currency in which the notes are denominated.  About two thirds of all public STS securitisations are denominated in EUR."""
